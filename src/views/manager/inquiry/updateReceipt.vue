...
<script>
  import { updateInquiryReceiptInfo, queryInquiryReceipt } from '@/api/inquiry';
  ...
  export default {
    ...
    methods: {
      submit() { // 改为修改的方法
        ...
        this.form.receiptId = this.tmpReceiptId;
        if (this.form.userId <= 0 || this.form.inquiryId <= 0 || this.form.receiptId <= 0) {
          this.$message({
            message: '存在userId 或 inquiryId 或 receiptId不大于0',
            type: 'warning'
          });
          this.isGo = false;
        } else { this.isGo = true; }
