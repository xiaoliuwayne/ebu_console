...
<script>
import { getSettleAccounts } from "@/api/financial";
...
export default {
  ...
  methods: {
    ...
    getData(params) {
      getSettleAccounts(params).then(res => {
        res.list.forEach(item => {
          ...
          item.customizeType =
            this.GLOBAL.customizeTypes[item.customizeType] || "-";
    ...
    // 跳转详情
    showDetail(id) {
      this.$router.push({ path: "settleAccountDetail", query: { id: id } });
    },
    // 切换页
